...
- `Git Commit Helper` (`git-commit-helper`): draft commit messages from staged diffs; adapt to this repo's commit message style
...
## Git / commits (OpenCode)

Trigger words:
- Only create commits when the user says: `comitea` (or explicitly asks to create a commit).
- Optional alias: `/commit` (documented in `.opencode/commands/commit.md`).

Commit policy:
- Split work into atomic commits (buildable/logical units), with clear intent-focused messages.
- Never commit secrets (e.g. `.env.local`) or credentials.
- Do not commit large exports like `data/*.csv` unless the user explicitly requests it.
- Do not edit generated files unless they are generated by the normal workflow; if they change, commit them in the same atomic unit that requires them (or their own commit if needed).

Atomic commit workflow when triggered:
1. Inspect: `git status`, `git diff`, and recent `git log` for message style.
2. Group changes by intent (e.g. rename domain model, auth wiring, UI copy, schema, generated types).
3. Stage the first group, commit, repeat until clean.
4. Run a fast verification step when reasonable (`bun run check` or `bun run test`) and report results.

Custom command config:
- `/commit` lives in `.opencode/commands/commit.md`.
